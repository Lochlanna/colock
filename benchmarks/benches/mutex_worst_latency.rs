...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for run in runs {
        ...
        group.bench_with_input(BenchmarkId::new("parking_lot", run), &run, |b, run| {
            b.iter_custom(|iters| {
                sync_shared::run_latency_benchmark::<parking_lot::Mutex<f64>>(run, iters)
            })
        });
        group.bench_with_input(BenchmarkId::new("usync", run), &run, |b, run| {
            b.iter_custom(|iters| {
                sync_shared::run_latency_benchmark::<usync::Mutex<f64>>(run, iters)
            })
        });
        group.bench_with_input(BenchmarkId::new("std", run), &run, |b, run| {
            b.iter_custom(|iters| {
                sync_shared::run_latency_benchmark::<std::sync::Mutex<f64>>(run, iters)
            })
        });
        group.bench_with_input(BenchmarkId::new("mini", run), &run, |b, run| {
            b.iter_custom(|iters| {
                sync_shared::run_latency_benchmark::<mini_lock::MiniLock<f64>>(run, iters)
            })
        });
        if cfg!(unix) {
            group.bench_with_input(BenchmarkId::new("pthread", run), &run, |b, run| {
                b.iter_custom(|iters| {
                    sync_shared::run_latency_benchmark::<PthreadMutex<f64>>(run, iters)
                })
            });
        }
        // if cfg!(windows) {
